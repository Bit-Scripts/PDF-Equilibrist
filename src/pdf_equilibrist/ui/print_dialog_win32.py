"""
ui/print_dialog_win32.py — Backend d'impression Win32 GDI direct
==================================================================
Importé uniquement sur Windows (voir print_dialog.py) — ``ctypes.wintypes``
n'existe pas sous Linux/macOS.

Contient le dialogue natif des propriétés du driver (``DocumentProperties``)
et le rendu d'impression bas niveau via ``winspool``/``gdi32`` avec la
stratégie hairline (upsampling + overlay vectoriel) pour les plans AutoCAD.
"""
...
def open_driver_properties(printer_name: str, hwnd: int,
...
def win32_print(printer_name: str, devmode_bytes: bytes | None,
