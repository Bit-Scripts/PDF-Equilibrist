...
# déjà réécrit vers le chemin d'install final par le .spec), plutôt que
...
# (même bug que sur le paquet PPA, voir packaging/ppa/pdf-equilibrist.sh).
exec /usr/lib64/pdf-equilibrist/venv/bin/pdf-equilibrist "$@"
